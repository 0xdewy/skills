...
RUNNER_FILES = {
    "run.json", "stderr.txt", "backend-output.jsonl", "grading.json",
    "evals.json", "summary.json",
}


def _fixture_manifest(base):
    run_path = base / "run.json"
    if not run_path.exists():
        return {}
    try:
        return json.loads(run_path.read_text()).get("fixture_manifest", {})
    except (OSError, ValueError, TypeError):
        return {}


def _is_unchanged_fixture(path, base, manifest):
    try:
        rel = path.relative_to(base / "workspace").as_posix()
    except ValueError:
        return False
    expected = manifest.get(rel, "__not_a_fixture__")
    if expected in (None, "__not_a_fixture__"):
...
def gather_artifacts(results_dir, eval_id):
    ...
    for base in candidates:
        if not base.exists():
            continue
        manifest = _fixture_manifest(base)
        for p in sorted(base.rglob("*")):
            if (p.is_file() and p.name not in RUNNER_FILES
                    and p.suffix in {".md", ".json", ".txt", ".py", ".sh", ".log", ".ts", ".js"}):
                if _is_unchanged_fixture(p, base, manifest):
                    continue
                try:
...
def call_judge(prompt, model):
    if model.startswith("codex:"):
        codex_model = model.split(":", 1)[1]
        try:
            res = subprocess.run(
                ["codex", "exec", "--ephemeral", "--json", "--skip-git-repo-check",
                 "-m", codex_model, prompt],
            ...
            if res.returncode != 0 or not messages:
                sys.stderr.write(f"  (codex CLI judge failed: {res.stderr.strip()})\n")
                return ""
            return "\n".join(messages)
        except Exception as e:
            sys.stderr.write(f"  (codex CLI judge unavailable: {e})\n")
            return ""
    try:
        import anthropic
...
def deterministic_checks(eval_obj, artifacts_text, eval_dir):
    held, failed = [], []
    ...
    for relpath in eval_obj.get("required_files", []) or []:
        exists = (eval_dir / relpath).exists() or (eval_dir / "workspace" / relpath).exists()
        (held if exists else failed).append(f"required_file {relpath!r}")
    return held, failed


def grade_eval(eval_obj, results_dir, model):
    eid = eval_obj.get("id", "?")
    eval_dir = results_dir / f"eval-{eid}"
    ...
    det_held, det_failed = deterministic_checks(eval_obj, artifacts_text, eval_dir)
    if det_failed:
        verdict["pass"] = False
