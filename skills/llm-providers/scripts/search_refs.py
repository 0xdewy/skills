#!/usr/bin/env python3
"""Deterministic local search over the llm-providers reference cards.

Lets an agent find the right reference file + heading for a query without
loading every file. Ranks matches from two sources:

  1. The alias table in references/source-map.json (includes common alternate
     spellings, SDK package names, and flagship model ids so an agent can find
     the right card from any phrasing).
  2. Markdown headings (and their first line of body) across references/*.md.

No network, stdlib only.

Usage:
    python scripts/search_refs.py "deepseek reasoning stream"
    python scripts/search_refs.py --json "openai compat base url"
    python scripts/search_refs.py --limit 3 "glm 4.6 thinking"
"""
...
def score(query_tokens: set[str], terms: list[str], phrase_text: str, query: str) -> float:
    if not query_tokens:
        return 0.0
    s = 0.0
    # Long tokens are exact-compared; only tokens of length >= 3 participate in
    # substring matching so single-letter tokens ("r", "v2", "a") don't fire
    # spurious matches on words that merely contain them.
    long_terms = [t for t in terms if len(t) >= 3]
    for qt in query_tokens:
        if qt in terms:
            s += 2.0
        elif len(qt) >= 3 and any(qt in t or t in qt for t in long_terms):
            s += 1.0
...
def search(query: str, limit: int) -> list[dict]:
    ...
    for e in load_alias_entries():
        ...
        flat_terms: list[str] = []
        for term in terms:
            flat_terms.extend(tokenize(term))
        phrase_text = " ".join(terms)
        sc = score(q_tokens, flat_terms, phrase_text, q)
        if sc > 0:
            results.append(
                {
                    "kind": "alias",
                    "file": e["file"],
                    "anchor": e.get("anchor", ""),
                    "heading": e.get("anchor", "").replace("-", " ").title() or e["file"],
                    "snippet": "matched terms: " + ", ".join(terms[:8]),
                    "source_url": e.get("source_url", ""),
                    "score": sc + 5.0,  # curated aliases rank above free-text headings
                }
            )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Search llm-providers reference cards.")
